(function () {
  ...
  function renderChart(rows, params, elapsedMin) {
    ...
    el.chart.innerHTML = `
      ...
      ${boilPath ? `<path d="${boilPath}" fill="none" style="stroke:var(--copper);stroke-width:2.5px;stroke-dasharray:6,3" stroke-linejoin="round" />` : ""}
  ...
  // Tema, cabeçalho e rodapé vêm do shell compartilhado da Brassagem Forte
  // (assets/js/shell.js) — o app só cuida da calculadora.
  ...
  function init() {
    ...
    // Service worker (offline) e aviso de nova versão vêm do shell
    // compartilhado (assets/js/shell.js + sw.js na raiz do site).
  }
