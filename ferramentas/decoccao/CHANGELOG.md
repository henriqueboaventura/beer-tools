...
### Alterado
- A calculadora passa a fazer parte do diretório **Ferramentas Brassagem
  Forte** (`/ferramentas/decoccao/`), com o layout compartilhado: header,
  menu, rodapé e tema claro/escuro vêm de `assets/js/shell.js`. O botão de
  tema próprio do app e o rodapé fixo no HTML saíram; o app só cuida da
  calculadora.
- Visual da Brassagem Forte: interface monocromática (preto, branco e
  cinza), com cor só onde ela carrega informação de temperatura — mostura
  (azul-aço), fervura (cobre), "agora" (âmbar), faixas de enzima, pontos da
  escada e pílulas de temperatura mantêm as cores originais. A fervura no
  gráfico também ficou tracejada, para não depender só da cor.
- `sobre.html` ("Por que decocção?") reescrita no mesmo layout;
  `styles.css` e `sobre.css` foram substituídos por `app.css`.
- PWA: o service worker, o manifest e os ícones próprios da calculadora
  saíram. Agora vale o PWA do site inteiro (`sw.js` na raiz), que também
  cuida do aviso de nova versão (antes era o toast "Nova versão ·
  Recarregar" do `app.js`, removido). A versão da calculadora continua em
  `version.js` e aparece no rodapé como "Calculadora v…".

### Corrigido
- Cronômetro no celular (`<=860px`) não grudava no topo ao rolar:
  `overflow-x: hidden` em `html`/`body` transformava a página num contêiner
  de rolagem e anulava o `position: sticky`. Trocado por `overflow-x: clip`,
  que continua cortando os tooltips que vazam para os lados.

### Sem mudança
- `methods.js`, `app-core.js` e os testes: nenhuma linha alterada. As
  chaves de `localStorage` (`decoccao:v1:*`) são as mesmas, e o domínio
  também, então predefinições e cronômetro da versão antiga continuam
  valendo.
